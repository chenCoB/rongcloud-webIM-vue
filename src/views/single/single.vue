                            ...
                            <template v-else>
                                <div class="chat-layout chat-opposite" v-if="value.senderUserId != userModel.user.id">
                                    <nut-avatar
                                            bgIcon=""
                                            style="flex: 0 0 32px"
                                            v-if="value.content.extra.sender"
                                            :bgImage="value.content.extra.sender.portrait"
                                    >
                                    </nut-avatar>
                                    <nut-avatar
                                            v-else
                                    ...
                                    <div v-if="value.messageType == 'ImageMessage'" class="messages-info-left">
                                        <div class="chat-user-name" v-if="value.content.extra">{{value.content.extra.sender.name}}</div>
                                ...
                                <div class="chat-layout chat-our" v-else>
                                    <nut-avatar
                                            ...
                                            v-if="userModel.user.portrait"
                                            bgIcon=""
                                            :bgImage="userModel.user.portrait"
                                    >
                                    </nut-avatar>
                                    <nut-avatar
                                            v-else
                                    ...
                                    <div v-if="value.messageType == 'ImageMessage'"  class="messages-info-right" >
                                        <div class="chat-user-name" v-if="userModel.user">{{ userModel.user.name}}</div>
                                        <img :src="imageAddress(value.content.imageUri)" @click.stop="detailsFigure(value.content.imageUri)"  alt="" width="100">
                                        <div class="active-positioning"  v-if="value.action">
                                            <ChatAction :recallMessage="value" @recallSucceed="recallSucceed"></ChatAction>
                                        </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.oppositeName = this.$route.query.oppositeName;
            this.targetPortrait = this.$route.query.portrait;
            this.type = this.$route.query.type == 1 ? 'PRIVATE' : 'GROUP';
            this.chatUser =  this.$route.query.chatUser
            this.init();
            if (this.status) {
        ...
        methods: {
            ...
            onUpFile(imageUrl) {
                ...
                this.dealImage(this.previewBase64, 300 , (base64)=>{
                    rongSendImage(this.type, base64, imageUrl, this.targetId, {
                        sender: this.userModel.user,
                        target: {
                            id: this.targetId,
                            name: this.oppositeName,
                            portrait: this.targetPortrait
                        }
                    }).then((message)=>{
            ...
            onSend() {
                ...
                if (this.content) {
                    ...
                    rongSendText(this.targetId,this.type,this.content,{
                        sender: this.userModel.user,
                        target: {
                            id: this.targetId,
                            name: this.oppositeName,
